...
@bp.route('/choose_course/add/<string:tid>')
def choose_course_for_add(tid):
    basic_info = fetch_basic_information(tid, 'teacher')
    results_course = fetch_tea_course(tid)
    # 点击添加分数/修改分数按钮后进入该函数
    return render_template('teacher/input_add_scores.html', courses=results_course, basic_info=basic_info)

@bp.route('/choose_course/add/<string:tid>')
def choose_course_for_modify(tid):
    # 点击修改分数按钮后进入该函数
    basic_info = fetch_basic_information(tid, 'teacher')
    results_course = fetch_tea_course(tid)
    return render_template('teacher/input_modify_scores.html', courses=results_course, basic_info=basic_info)
...
def add_scores_view(tid):
    ...
    if _cid and _semester:
        ...
        return render_template('teacher/tea_add_scores.html', tid=tid, cid=_cid, sem=_semester, res=results)
    else:
